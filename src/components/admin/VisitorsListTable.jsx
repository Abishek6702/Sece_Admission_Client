...
const VisitorsListTable = () => {
  ...
  const [revisitRow, setRevisitRow] = useState(null);
  const [exporting, setExporting] = useState(false);

  const [revisitDate, setRevisitDate] = useState(
    () => new Date().toISOString().split("T")[0]
  );
  const [visitedBy, setVisitedBy] = useState("Chairman");
      ...
      ${row.status === "Rejected" ? "bg-red-100 text-red-700" : ""}`}
                ...
                <td className="px-5 py-3 flex items-center space-x-2 justify-center">
                  <button
                    ...
                    onClick={() => {
                      setRevisitRow(row); // store current row
                      setRevisitDate(new Date().toISOString().split("T")[0]);
                      setVisitedBy("Chairman");
                  ...
                  <button
                    onClick={() => navigate(`/admin/visitors_list/${row._id}`)}
                    className="p-1.5 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-200"
                  >
                    <Eye size={14} />
                  </button>
      ...
      {revisitModalOpen && (
        <div className="fixed inset-0 tint flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-lg max-w-sm w-full">
            <h2 className="text-lg font-semibold text-[#282526] mb-2">
              Revisit Entry
            </h2>
            ...
            <div className="flex justify-end gap-3 mt-3">
              ...
              <button
                className={`px-3 py-1 rounded text-white ${
                  revisitLoading
                    ? "bg-[#0b56a4]/50 cursor-not-allowed"
                    : "bg-[#0b56a4] cursor-pointer"
                }`}
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (!response.ok) {
                      throw new Error("Failed to mark revisit");
                    }

                    const data = await response.json();
                    console.log("Revisit response:", data);
                    toast.success("Revisit marked successfully!");
                    setTimeout(() => window.location.reload(), 2000);

                    setRevisitModalOpen(false);
                  } catch (error) {
                    toast.error("Failed to mark revisit");
                  } finally {
                ...
                disabled={revisitLoading}
              >
                {revisitLoading ? "Submitting..." : "Submit"}
              </button>
            </div>
          </div>
        </div>
...
export default VisitorsListTable;
