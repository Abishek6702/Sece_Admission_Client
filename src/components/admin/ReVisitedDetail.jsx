...
export default function ReVisitedDetail() {
  const { id } = useParams();
    ...
    <div className=" max-w-6xl ">
      ...
      <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
        <section className="bg-white px-5 py-3 shadow rounded-lg">
          <h2 className="text-xl font-semibold text-[#0B56A4] mb-4 playfair">
            ReVisit Details
          </h2>

          {enquiry.revisits && enquiry.revisits.length > 0 ? (
            <table className="min-w-full text-sm text-left text-gray-700">
              ...
              <tbody>
                {enquiry.revisits.map((revisit, index) => (
                  <tr
                    key={index}
                    className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}
                  >
                    <td className=" px-4 py-2">
                      {new Date(revisit.date).toLocaleDateString()}
                    </td>
                    <td className=" px-4 py-2">
                      {revisit.visitedBy}
                    </td>
            ...
            <p className="text-gray-500 text-sm">No revisit records found.</p>
          )}
        </section>
