...
export default function CourseTable() {
  ...
  // bulk status chnage
  const bulkChangeStatus = async (selectedIds, newStatus) => {
    try {
      // console.log("Bulk Change Status for IDs:", selectedIds, "to", newStatus);

      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/enquiries/bulk/status`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      {modalOpen && (
        <div className="fixed inset-0 tint flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-lg max-w-sm w-full">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-[#282526]">
                Change Status for {selectedRows.length} Selected
              </h2>
              <button
                className="bg-gray-300 rounded-2xl p-2 flex items-center justify-center cursor-pointer"
                onClick={() => {
                  setNewStatus(null);
                  setModalOpen(false);
                }}
            ...
            <div className="space-y-3 mb-4">
              ...
              >
                <input
                  ...
                  name="status"
                  value="Selected"
                  checked={newStatus === "Selected"}
                  onChange={() => setNewStatus("Selected")}
                  className="hidden"
                />
                <span
              ...
              <button
                ...
                className={`px-3 py-1 rounded text-white ${
                  newStatus
                    ...
                    : "bg-[#0b55a45e] cursor-not-allowed"
                }`}
                onClick={() => {
                  bulkChangeStatus(selectedRows, newStatus);
                  setModalOpen(false);
                  setNewStatus(null);
                }}
              >
