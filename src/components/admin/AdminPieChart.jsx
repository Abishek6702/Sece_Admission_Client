import React, { useState, useEffect } from "react";
import { Doughnut } from "react-chartjs-2";
import { Chart as ChartJS, ArcElement, Tooltip, Legend } from "chart.js";

ChartJS.register(ArcElement, Tooltip, Legend);

const AdminPieChart = () => {
  const [selectedYear, setSelectedYear] = useState(null);
  const [yearlyData, setYearlyData] = useState({});

  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_BASE_URL}/api/dashboard/yearly-quota-counts`)
      .then((res) => res.json())
      .then((data) => {
        const transformedData = {};
        Object.entries(data).forEach(([year, quotas]) => {
          transformedData[year] = {
            labels: ["Management Quota", "Government Quota"],
            data: [
              quotas["Management Quota"] || 0,
              quotas["Government Quota"] || 0,
            ],
          };
        });
        setYearlyData(transformedData);

        // Set selectedYear to first available year dynamically
        const firstYear = Object.keys(transformedData)[0];
        if (firstYear) setSelectedYear(firstYear);
      })
      .catch((err) => console.error("Error loading yearly quota counts", err));
  }, []);

  const years = Object.keys(yearlyData);

  const chartData = selectedYear && yearlyData[selectedYear] ? {
    labels: yearlyData[selectedYear].labels,
    datasets: [
      {
        label: `Quota Distribution - ${selectedYear}`,
        data: yearlyData[selectedYear].data,
        backgroundColor: ["#2e9797", "#ff6384"],
        hoverBackgroundColor: ["#4bc0c0", "#ff6384"],
        borderWidth: 0,
      },
    ],
  } : { labels: [], datasets: [] };

  const options = {
    cutout: "70%",
    plugins: {
      legend: {
        position: "bottom",
        display: false,
      },
      tooltip: {
        enabled: true,
        callbacks: {
          label: function (context) {
            const label = context.label || "";
            const value = context.parsed || 0;
            return `${label}: ${value}`;
          },
        },
      },
    },
    maintainAspectRatio: false,
  };

  if (!selectedYear) {
    return <div>Loading chart data...</div>;
  }

  return (
    <div className="h-full bg-white rounded-xl shadow-lg">
      <div className="h-70 lg:w-75 lg:mt-0 rounded-xl p-6 flex flex-col items-center">
        <div className="playfair w-full font-semibold text-[#282526] flex items-center justify-between mb-6">
          <h2 className="playfair text-xl font-bold text-[#282526]">
            Total Student&apos;s
          </h2>
          <div className="roboto">
            <select
              className="border border-gray-300 text-[#282526] font-semibold rounded-lg p-1 px-2 outline-none"
              value={selectedYear}
              onChange={(e) => setSelectedYear(e.target.value)}
            >
              {years.map((year) => (
                <option key={year} value={year}>
                  {year}
                </option>
              ))}
            </select>
          </div>
        </div>

        <div className="w-full h-full">
          <Doughnut data={chartData} options={options} />
        </div>
      </div>
      <div className="w-[100%] text-center space-y-2">
        <div className="flex items-center justify-center gap-3">
          <span className="w-6 h-3 rounded-md bg-[#4bc0c0] shadow-lg"></span>
          <span className="font-semibold text-[#282526] text-lg">
            Management Quota
          </span>
        </div>
        <div className="flex items-center justify-center gap-3">
          <span className="w-6 h-3 rounded-md bg-[#ff6384] shadow-lg"></span>
          <span className="font-semibold text-[#282526] text-lg">
            Government Quota
          </span>
        </div>
      </div>
    </div>
  );
};

export default AdminPieChart;
