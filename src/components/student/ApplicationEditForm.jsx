...
const ApplicationEditForm = () => {
    ...
    const fetchApplication = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/api/application/user/${userId}`
        );
        ...
        if (res.ok && result.success) {
          ...
          // // ✅ Load existing photo reasons if they exist
          // setPhotoReasons({
          //   fatherPhotoReason: result.data.fatherPhotoReason || "",
          //   motherPhotoReason: result.data.motherPhotoReason || "",
          // });
        } else {
  ...
  useEffect(() => {
    if (data.status === "Remark") {
      setDisabled(false); // editable
    } else {
      setDisabled(true); // read-only
    }
  }, [data.status]);
  ...
  const handleReasonSubmit = () => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      // ✅ Proceed with submission with reasons
      submitApplication(photoReasons.fatherPhotoReason, photoReasons.motherPhotoReason);
    }
  };

  // ✅ Modified to accept photo reasons as parameters
  const submitApplication = async (fatherReason = "", motherReason = "") => {
    ...
    try {
      ...
      for (const key in data) {
        // Skip remarks and photo reasons (handled separately)
        if (key === "remarks" || key === "fatherPhotoReason" || key === "motherPhotoReason") continue;
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/application/${applicationId}/resubmit`,
        {
          method: "PUT",
          body: formData,
        }
  ...
  const handleStepSubmit = async (e) => {
    ...
    if (Object.keys(currentErrors).length === 0) {
      if (step < 5) {
        ...
        // ✅ Check if parent photos are missing on final step
    ...
    <>
      <div className="max-w-4xl mx-auto">
        ...
        <form onSubmit={handleStepSubmit}>
          <fieldset
            disabled={disabled}
            className={disabled ? "[&_*]:cursor-not-allowed" : ""}
          >
            {step === 0 && (
              <PersonalDetailStep
                data={data}
